...
module Mrmime_parsetree = struct
  ...
  let content_type hd =
    ...
    let params =
      let form (name, value) =
        Header.Field.Value.Parameter.make_ name
          ( match value with
          | `String s -> s
          | `Token s -> s )
  ...
  module Make (T : PARSETREE) = struct
    ...
    let create_new_header md =
      ...
      let cd_header_val =
        ...
        let params = [ ("filename", md.filename) ] in
        Header.Field.Value.make
          ~params:
            (map
               (uncurry Header.Field.Value.Parameter.make)
    ...
    let already_converted tree =
      ...
      let process att =
        let ( let* ) = Option.( >>= ) in
        let* meta =
          T.meta_val (T.header (T.of_attachment att))
        in
        let* hashed =
      ...
      let rec process l r =
        ...
        | (h, id) :: hs ->
          ...
          then
            process hs
              (List.filter
                 (fun c -> TransformData.convert_id c <> id)
                 r )
          else process hs r
      in
    ...
    let convert_attachments ?(idem = true) dict tree pbar =
      let open Configuration in
      let done_converting =
        if idem then already_converted tree else []
      in
      let process att =
        if (not idem) || not (is_converted att)
        then
          match T.content_type (Attachment.header att) with
          | None -> []
          | Some ct_hv -> (
            match
              Mime_type.of_string
                (Header.Field.Value.value ct_hv)
            with
            | Error _ -> []
            | Ok mty ->
              let hashed =
                Hashtbl.hash (Attachment.data att)
              in
              let conversions =
                Formats.conversions dict mty
              in
              let trans_lst =
                filter_converted hashed done_converting
                  conversions
              in
