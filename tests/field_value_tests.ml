...
let of_string_test1 =
  ...
  let expected = V.make ~params:[ param1; param2 ] "attachment" in
...
let to_string_test1 =
  check_eq_basic "basic to_string of basic_cont_dis test"
    (Ok basic_cont_dis) (Result.map V.to_string cd)
...
let update_tests =
  ...
  let test2 =
    check_eq_basic "basic update_param test"
      (Result.map (V.make ~params:[ param1; param3 ] << V.value) cd)
      (Result.map (V.update_param "filename" (fun _ -> Some param3)) cd)
  in
  let test3 =
    check_eq_basic "basic update_param remove test"
      (Result.map (V.make ~params:[ param1 ] << V.value) cd)
      (Result.map (V.update_param "filename" (fun _ -> None)) cd)
  in
  let test4 =
    check_eq_basic "basic update_param no op test" cd
      (Result.map (V.update_param "file" (fun _ -> None)) cd)
  in
  let test5 =
    check_eq_basic "basic add_param test"
      (Result.map (V.make
         ...
         << V.value) cd)
      (Result.map (V.add_param ~quotes:true "whatever" "ok.gif") cd)
  in
  let test6 =
