...
let of_string_test1 =
  ...
  check_eq_basic
    "basic of_string of basic_cont_dis test"
...
let update_tests =
  let param1: Parameter.t = { attr = "filename*"; value = "utf-8''test.gif"; quotes = false } in
  let param2: Parameter.t = { attr = "filename"; value = "test.gif"; quotes = true } in
  let param3: Parameter.t = { attr = "filename"; value = "ok.gif"; quotes=false } in
  let param4: Parameter.t = { attr = "whatever"; value = "ok.gif"; quotes=false } in
   let test1 =
    check_eq_basic
      "basic update_value test"
      { value = "inline"; params = [param1 ; param2 ] }
      (update_value "inline" cd)
  in
  let test2 =
    check_eq_basic
      "basic update_param test"
      { cd with params = [param1 ; param3 ] }
      (update_param "filename" (fun _ -> (Some param3)) cd)
  in
  let test3 =
    check_eq_basic
      "basic update_param remove test"
      { cd with params = [param1] }
      (update_param "filename" (fun _ -> None) cd)
  in
  let test4 =
    check_eq_basic
      "basic update_param no op test"
      cd
      (update_param "file" (fun _ -> None) cd)
  in
  let test5 =
    check_eq_basic
      "basic add_param test"
      { cd with params = [param1 ; param2 ; param4] }
      (add_param "whatever" "ok.gif" cd)
  in
  let test6 =
    check_eq_basic
      "basic add_param with quotes test"
      { cd with params = [param1 ; param2 ; { param4 with quotes=true } ] }
      (add_param ~quotes:true "whatever" "ok.gif" cd)
  in
  let test7 =
    check_eq_basic
      "add_param as update test"
      { cd with params = [param1 ; param3 ] }
