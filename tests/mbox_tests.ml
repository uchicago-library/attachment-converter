...
let dummy_mbox = to_mbox ~eol:"\r\n" ["Email 1"; "Email 2"; "Email 3"]
let dummy_mbox_expected = "From BLAH\r\nEmail 1\r\nFrom BLAH\r\nEmail 2\r\nFrom BLAH\r\nEmail 3\r\n"
...
let basic_convert_test f expected description =
  let module T = Conversion (MBoxIterator(StringInput)) (StringOutput) in
...
let convert_test_2 =
  basic_convert_test (fun (x, y) -> x ^ "\r\n" ^ y) dummy_mbox_expected
...
let convert_test_3 =
  ...
  let email = "testing\r\ntesting" in
  let mbox = to_mbox ~eol:"\r\n" [email; email] in
  let output = T.convert mbox () (fun (x, y) -> x ^ "\r\n" ^ y) in
