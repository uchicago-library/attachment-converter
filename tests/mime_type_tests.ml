...
let type_ =
  Alcotest.testable Mime_type.Type.pp Mime_type.Type.equal
...
let mime_type =
  Alcotest.testable Mime_type.pp Mime_type.equal
...
let test_type_of_string_case_sensitivity () =
  ...
  let act =
    Result.to_option
      (Mime_type.Type.of_string "APPlicatioN")
  in
...
let test_of_string_case_senitivity () =
  ...
  let act =
    Result.to_option (Mime_type.of_string "APPliCATion/PDf")
  in
...
let test_make () =
  ...
  let msg = "same type" in
  let exp =
    Result.to_option (Mime_type.of_string "model/mesh")
  in
  let act =
