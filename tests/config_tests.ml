...
(* check_is_error (ConfigEntry.of_refer nwf)
   "entry_of_assoc" *)

(* let check_entry l description st tt ss = *)
(*   let open ConfigEntry in *)
(*   let l_parsed = Result.get_ok (of_refer l) in *)
(* let st_check _ = assert_equal (source_type l_parsed) st
   in *)
(* let tt_check _ = assert_equal (target_type l_parsed) tt
   in *)
(*   let ss_check _ = *)
...
(* ~target_ext:"q" ~shell_command:"c d e"
   ~convert_id:"id" *)
...
(* let description = "checking access for wf_cs/extra_cs"
   in *)
(*   let open Formats in *)
...
(*   in *)
(* check_eq_basic "Error not as expected, wanted
   ConfigData" *)
(*     error *)
