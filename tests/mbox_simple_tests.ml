...
let test_read_valid_message _ =
  ...
  | Error e ->
    assert_failure ("Unexpected error: " ^ e) ;
    close_in ic
...
let test_malformed_start _ =
  ...
  match MBoxParser.read mchan true with
  | Error msg ->
    assert_bool "Error should describe malformed start"
      (String.contains msg 'm')
  | Ok _ ->
    assert_failure "Expected failure due to malformed input" ;
    close_in ic
...
let test_convert_applies_function _ =
  ...
  let fd_out =
    Unix.openfile tmp_out
      [ Unix.O_WRONLY; Unix.O_TRUNC; Unix.O_CREAT ]
      0o600
  in
  Unix.dup2 fd_out Unix.stdout ;
  let result =
    MBoxParser.convert ic (fun s ->
  ...
  let output =
    really_input_string oc (in_channel_length oc)
  in
  close_in oc ;
  assert_bool "Output should contain uppercase FROM"
    (String.contains output 'F') ;
  assert_bool "Output should contain uppercase HELLO"
    (String.contains output 'H') ;
  close_in ic
...
let test_fold_handles_errors _ =
  ...
  in
  let results =
    MBoxParser.fold (fun acc r -> r :: acc) ic []
    |> List.rev
  in
  let oks =
    List.filter
...
let suite =
  ...
  >::: [ "read_valid_message" >:: test_read_valid_message;
         ...
         "fold_multiple_messages"
         >:: test_fold_multiple_messages;
         "convert_applies_function"
         >:: test_convert_applies_function;
